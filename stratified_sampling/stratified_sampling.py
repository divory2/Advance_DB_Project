...
def getAproxSumAgregation(df,groupByCategory,val_col,fraction,get_aggregation_result):
    ...
    "execution time": executionTime,
    "approx result": aproximateSampleResults,
...
def getAproxMedianAgregation(df,groupByCategory,val_col,fraction,get_aggregation_result):
    ...
    start_time = time.perf_counter()
    stratatifiedSample = df.groupby(groupByCategory).sample(frac=fraction)
    aproximateSampleResults = stratatifiedSample[val_col].median()
    end_time = end_time = time.perf_counter()
...
def getAproxAvgAgregation(df,groupByCategory,val_col,fraction,get_aggregation_result):
    ...
    stratatifiedSample = df.groupby(groupByCategory).sample(frac=fraction)
    aproximateSampleResults = stratatifiedSample[val_col].mean()
...
"""first define main and start to structure the flow of 
work flow
"""
def main():
